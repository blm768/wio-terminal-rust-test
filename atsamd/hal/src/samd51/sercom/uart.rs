...
macro_rules! uart {
    ($Type:ident: (
        ...
        $crate::paste::item! {
            ...
            /// The write half of the corresponding UARTX instance (as returned by `UARTX::split`)
            ...
            impl<TX, RTS> [<$Type Tx>]<TX, RTS> {
                fn usart(&self) -> &USART_INT {
                    // SAFETY: only this struct instance should be able to access TX-related fields on this SERCOM.
                    unsafe { (*$SERCOM::ptr()).usart_int() }
                }
            ...
            impl<TX, RTS> serial::Write<u8> for [<$Type Tx>]<TX, RTS> {
                ...
                fn write(&mut self, word: u8) -> nb::Result<(), Self::Error> {
                    Self::do_write(self.usart(), word)
                }

                fn flush(&mut self) -> nb::Result<(), Self::Error> {
                    Self::do_flush(self.usart())
                }
            }
            ...
            /// The read half of the corresponding UARTX instance (as returned by `UARTX::split`)
            ...
            impl<RX, CTS> [<$Type Rx>]<RX, CTS> {
                fn usart(&self) -> &USART_INT {
                    // SAFETY: only this struct instance should be able to access RX-related fields on this SERCOM.
                    unsafe { (*$SERCOM::ptr()).usart_int() }
                }
